...
var tenantId = os.Getenv("ARM_TEST_B2C_TENANT_ID")
	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			...
			Check: resource.ComposeTestCheckFunc(
				check.WithTenant(tenantId).That(data.ResourceName).ExistsInAzure(r),
			),
		},
	})
}
...
func (r B2CUserFlowResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	client := clients.UserFlows.B2CUserFlowClient
...
`, data.TenantID, data.RandomString)
}
