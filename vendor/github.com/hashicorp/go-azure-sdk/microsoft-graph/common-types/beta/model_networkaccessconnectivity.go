...
func (s NetworkaccessConnectivity) Entity() BaseEntityImpl {
...
var _ json.Marshaler = NetworkaccessConnectivity{}

func (s NetworkaccessConnectivity) MarshalJSON() ([]byte, error) {
	type wrapper NetworkaccessConnectivity
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling NetworkaccessConnectivity: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling NetworkaccessConnectivity: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.networkaccess.connectivity"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling NetworkaccessConnectivity: %+v", err)
	}
