...
func (s RawTeamsAppInstallationImpl) MarshalJSON() ([]byte, error) {
...
func (s *BaseTeamsAppInstallationImpl) UnmarshalJSON(bytes []byte) error {
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling BaseTeamsAppInstallationImpl into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["scopeInfo"]; ok {
		impl, err := UnmarshalTeamsAppInstallationScopeInfoImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'ScopeInfo' for 'BaseTeamsAppInstallationImpl': %+v", err)
		}
		s.ScopeInfo = impl
	}
