...
func (s RawExtensionImpl) MarshalJSON() ([]byte, error) {
...
func UnmarshalExtensionImplementation(input []byte) (Extension, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.personExtension") {
		var out PersonExtension
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into PersonExtension: %+v", err)
		}
		return out, nil
	}
