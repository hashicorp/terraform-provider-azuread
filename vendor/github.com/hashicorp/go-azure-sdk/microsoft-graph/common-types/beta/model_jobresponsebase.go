...
func (s RawJobResponseBaseImpl) MarshalJSON() ([]byte, error) {
...
var _ json.Unmarshaler = &BaseJobResponseBaseImpl{}

func (s *BaseJobResponseBaseImpl) UnmarshalJSON(bytes []byte) error {
	var decoded struct {
		CreationDateTime nullable.Type[string] `json:"creationDateTime,omitempty"`
		EndDateTime      nullable.Type[string] `json:"endDateTime,omitempty"`
		StartDateTime    nullable.Type[string] `json:"startDateTime,omitempty"`
	...
	if err := json.Unmarshal(bytes, &temp); err != nil {
		return fmt.Errorf("unmarshaling BaseJobResponseBaseImpl into map[string]json.RawMessage: %+v", err)
	}

	if v, ok := temp["error"]; ok {
		impl, err := UnmarshalClassificationErrorImplementation(v)
		if err != nil {
			return fmt.Errorf("unmarshaling field 'Error' for 'BaseJobResponseBaseImpl': %+v", err)
		}
		s.Error = &impl
	}
