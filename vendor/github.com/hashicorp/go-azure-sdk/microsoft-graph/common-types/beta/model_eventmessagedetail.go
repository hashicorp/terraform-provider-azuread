...
func UnmarshalEventMessageDetailImplementation(input []byte) (EventMessageDetail, error) {
	...
	if strings.EqualFold(value, "#microsoft.graph.channelSharingUpdatedEventMessageDetail") {
		var out ChannelSharingUpdatedEventMessageDetail
		if err := json.Unmarshal(input, &out); err != nil {
			return nil, fmt.Errorf("unmarshaling into ChannelSharingUpdatedEventMessageDetail: %+v", err)
		}
		return out, nil
	}
