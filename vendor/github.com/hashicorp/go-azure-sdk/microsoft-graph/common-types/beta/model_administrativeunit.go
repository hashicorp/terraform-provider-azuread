...
type AdministrativeUnit struct {
	DeletedMembers *[]DirectoryObject `json:"deletedMembers,omitempty"`

	// List of OData IDs for `DeletedMembers` to bind to this entity
	DeletedMembers_ODataBind *[]string `json:"deletedMembers@odata.bind,omitempty"`
...
func (s *AdministrativeUnit) UnmarshalJSON(bytes []byte) error {
	...
	s.DeletedMembers_ODataBind = decoded.DeletedMembers_ODataBind
	...
	if v, ok := temp["deletedMembers"]; ok {
		var listTemp []json.RawMessage
		if err := json.Unmarshal(v, &listTemp); err != nil {
			return fmt.Errorf("unmarshaling DeletedMembers into list []json.RawMessage: %+v", err)
		}
		...
		for i, val := range listTemp {
			...
			if err != nil {
				return fmt.Errorf("unmarshaling index %d field 'DeletedMembers' for 'AdministrativeUnit': %+v", i, err)
			}
			output = append(output, impl)
		}
		s.DeletedMembers = &output
	}
