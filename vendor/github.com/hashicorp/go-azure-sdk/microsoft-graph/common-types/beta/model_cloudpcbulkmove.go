...
var _ CloudPCBulkAction = CloudPCBulkMove{}

type CloudPCBulkMove struct {
...
func (s CloudPCBulkMove) CloudPCBulkAction() BaseCloudPCBulkActionImpl {
...
func (s CloudPCBulkMove) Entity() BaseEntityImpl {
...
var _ json.Marshaler = CloudPCBulkMove{}

func (s CloudPCBulkMove) MarshalJSON() ([]byte, error) {
	type wrapper CloudPCBulkMove
	wrapped := wrapper(s)
	encoded, err := json.Marshal(wrapped)
	if err != nil {
		return nil, fmt.Errorf("marshaling CloudPCBulkMove: %+v", err)
	}

	var decoded map[string]interface{}
	if err = json.Unmarshal(encoded, &decoded); err != nil {
		return nil, fmt.Errorf("unmarshaling CloudPCBulkMove: %+v", err)
	}

	if !s.OmitDiscriminatedValue {
		decoded["@odata.type"] = "#microsoft.graph.cloudPcBulkMove"
	}

	encoded, err = json.Marshal(decoded)
	if err != nil {
		return nil, fmt.Errorf("re-marshaling CloudPCBulkMove: %+v", err)
	}
