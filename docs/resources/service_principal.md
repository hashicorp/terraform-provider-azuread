...
* `tags` - (Optional) A set of tags to apply to the service principal, for configuring specific behaviors of the service principal. Note that these are not provided for use by practitioners. Cannot be used together with the `feature_tags` block.
