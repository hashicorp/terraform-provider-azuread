...
* `tags` - (Optional) A set of tags to apply to the application, for configuring specific behaviors of the application and linked service principals. Note that these are not provided for use by practitioners. Cannot be used together with the `feature_tags` block.
